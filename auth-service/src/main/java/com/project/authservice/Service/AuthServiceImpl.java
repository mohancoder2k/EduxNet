...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public TokenValidationResponse checkToken(String token) {
        try {
            String email = jwtUtil.extractUsername(token);
            boolean isValid = jwtUtil.validateToken(token, userRepository.findByEmail(email).orElse(null));
            return new TokenValidationResponse(email, isValid);
        } catch (Exception e) {
            return new TokenValidationResponse(null, false);
        }
    }
